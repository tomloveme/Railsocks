...
RUN apt-get update && \
    apt-get install -y wget build-essential libssl-dev libev-dev libc-dev libpcre3-dev libsodium-dev libmbedtls-dev autoconf automake libtool && \
    ...
    make install && \
    cd .. && \
    rm -rf shadowsocks-libev-3.3.5 shadowsocks-libev-3.3.5.tar.gz
